...
public sealed class NetworkManager
{
    ...
    public Action<int> OnScoreReceived;
    ...
    {
        var request = new CSBaseLib.PKTReqUserScoreGet() { UserID = userID };

        var Body = MessagePackSerializer.Serialize(request);
        // PACKETID.REQ_USER_SCORE_GET 이 ID를 PacketDefine.cs에 추가해야 합니다.
        var sendData = CSBaseLib.PacketToBytes.Make(CSBaseLib.PACKETID.REQ_USER_SCORE_GET, Body);

        PostSendPacket(sendData);
        Debug.Log($"서버에 점수 요청: UserID={userID}");
    }
    ...
    {
        // 여기서 "Test"는 로그인 시점에 할당된 실제 UserID로 대체되어야 합니다.
        var request = new CSBaseLib.PKTReqUserScoreAdd() { UserID = "Test", AddScore = scoreIncrease };

        var Body = MessagePackSerializer.Serialize(request);
        // PacketID는 PacketDefine.cs에 정의한 REQ_USER_SCORE_ADD를 사용해야 합니다.
        var sendData = CSBaseLib.PacketToBytes.Make(CSBaseLib.PACKETID.REQ_USER_SCORE_ADD, Body);
    ...
    void PacketProcess(PacketData packet)
    {
        switch ((PACKETID)packet.PacketID)
        {
            ...
            case PACKETID.RES_USER_SCORE_UPDATE:
                ...
                break;
            case PACKETID.RES_USER_SCORE_GET:
                {
                    var resData = MessagePackSerializer.Deserialize<PKTResUserScoreGet>(packet.BodyData);

                    if (resData.Result == (short)ERROR_CODE.NONE)
                    {
                        Debug.Log($"서버 응답: 점수 획득 성공! 점수: {resData.Score}");

                        // --- 이 부분을 활성화 및 수정합니다 ---
                        // UnityMainThreadDispatcher가 씬에 존재하고 Instance가 설정되었다고 가정합니다.
                        if (UnityMainThreadDispatcher.Instance != null)
                        {
                            UnityMainThreadDispatcher.Instance.Enqueue(() => {
                                // 메인 스레드에서 OnScoreReceived 이벤트를 실행
                                NetworkManager.Instance.OnScoreReceived?.Invoke(resData.Score);
                            });
                        }
                    }
                    // ... (나머지 코드) ...
            ...
            case PACKETID.RES_USER_SCORE_ADD: // 새로운 점수 추가 응답 처리
                {
                    var resData = MessagePackSerializer.Deserialize<PKTResUserScoreAdd>(packet.BodyData);

                    if (resData.Result == (short)ERROR_CODE.NONE)
                    {
                        Debug.Log($"서버 응답: 점수 추가 성공! 최종 점수: {resData.FinalScore}");

                        // UnityMainThreadDispatcher를 통해 최종 점수를 UI로 전달
                        if (UnityMainThreadDispatcher.Instance != null)
                        {
                            UnityMainThreadDispatcher.Instance.Enqueue(() => {
                                // UI 스크립트(ScoreDisplay)에 등록된 이벤트 호출
